...
/// لینک دعوت ذخیره‌شده‌ی یک گروه را برمی‌گرداند (اگه از قبل ساخته شده باشه)، بدون زدن API تلگرام.
/// برای جلوگیری از باطل‌شدن مکرر لینک قبلی، همیشه قبل از export_chat_invite_link این را چک کن.
pub fn get_group_join_link(telegram_chat_id: i64) -> Result<Option<String>> {
    let conn = get_conn()?;
    let link: Option<String> = conn
        .query_row(
            "SELECT join_link FROM groups WHERE telegram_chat_id = ?1",
            params![telegram_chat_id],
            |row| row.get(0),
        )
        .unwrap_or(None);
...
pub fn is_user_restricted(group_id: i64, user_id: i64, lock_type: &str) -> Result<bool> {
    // مالکین (اول و بعدی)، ادمین‌های ربات، و VIP از همه قفل‌ها معاف‌اند
    if is_first_owner(group_id, user_id)?
        || is_owner(group_id, user_id)?
        || is_group_admin(group_id, user_id)?
        || is_vip(group_id, user_id)?
    {
        return Ok(false);
    }
